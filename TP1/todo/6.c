...
//Inverts the pixels
void invert(u8 *in, u64 len, u8 *out)
{
  //
  for (u64 i = 0; i < len; i++)
    out[i] = 255 - in[i];
...
int main(int argc, char **argv)
{
  ...
  //
  ppm_t *p_out = create_ppm(p_in->w, p_in->h, p_in->t);

  if (!p_out)
    exit(-1);

  //
  invert(p_in->pixels, p_in->w * p_in->h * 3, p_out->pixels);

  //
  write_ppm(p_out, "out.ppm");

  //
  release_ppm(p_in); free(p_in);
  release_ppm(p_out); free(p_out);
